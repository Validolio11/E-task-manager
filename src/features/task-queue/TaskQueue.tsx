    ...
    <div className="queue-scroll">
      <section><div className="queue-section-title"><span>ДАЛІ</span><strong>{open.length}</strong></div>{open.map((task) => {
        ...
        return <article className={`${state.selectedTaskId === task.id ? "selected" : ""} ${active ? "active" : ""}`} key={task.id}>
          ...
          <button type="button" className="queue-more" onClick={() => setMenuId((id) => id === task.id ? null : task.id)} aria-label={`Дії задачі ${task.title}`} aria-haspopup="menu" aria-expanded={menuId === task.id}><MoreHorizontal/></button>
          {menuId === task.id && <div className="task-popover" role="menu"><button role="menuitem" onClick={() => { setMenuId(null); onEdit(task); }}><Pencil/>Редагувати</button><button role="menuitem" onClick={() => { setMenuId(null); completeTask(task.id); }}><Check/>Завершити</button><button role="menuitem" className="danger-item" onClick={() => { setMenuId(null); onDelete(task); }}><Trash2/>Видалити</button></div>}
        </article>;
      })}</section>
